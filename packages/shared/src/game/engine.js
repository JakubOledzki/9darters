...
function createPlayerState(player, order, mode) {
    return {
...
function updateTurnBuckets(player, total) {
    if (total === 180) {
...
function finishMatch(state, winnerIndex) {
    state.status = "finished";
    state.winnerIndex = winnerIndex;
    state.finishedAt = new Date().toISOString();
    state.pendingThrows = [];
}
function winLegOrMatch(state, winnerIndex, checkoutValue) {
...
function commitCricketTurn(state) {
    const player = state.players[state.currentPlayerIndex];
    ...
    for (const entry of state.pendingThrows) {
        ...
        if (!CRICKET_TARGETS.includes(String(segment))) {
            continue;
        }
...
function commitAroundTheClockTurn(state) {
    const player = state.players[state.currentPlayerIndex];
    let hits = 0;
    const dartsUsed = countDarts(state.pendingThrows, state.pendingThrows.length || 3);
    player.stats.dartsThrown += dartsUsed;
...
export function createInitialMatchState(config, players, starterIndex = 0) {
...
export function pushThrow(state, entry) {
...
export function clearPendingThrows(state) {
    return {
        ...state,
        pendingThrows: []
    };
}
export function beginLiveMatch(state) {
...
export function commitTurn(state) {
    ...
    const timelineEntry = {
        ...
        dartsUsed: countDarts(state.pendingThrows, state.config.countingMode === "default" ? 3 : state.pendingThrows.length || 3),
